...
async def handle_send_check_result(message: types.Message, command: CommandObject) -> None:
    """Handle /check command - search for author in blacklist database."""
    ...
    try:
        if command.args is None:
            await message.reply(
                '❌ Команду введено неправильно. Ось приклад:\n\n'
        ...
        if search:
            ...
            final_message = (
                f'🙄 Ой йой... Здається я дещо знайшов:\n\n{formatted_results}\n\n'
            ...
            await message.reply(
                '😮‍💨 На щастя - нічого не знайдено!\n'
                'Але радимо додатково перевіряти авторів'
            )
    except TelegramNetworkError:
        await message.reply(
            '🥀 🥀 🥀 Вибачте, сталась помилка на стороні Telegram:\n\n'
