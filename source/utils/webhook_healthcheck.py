...
WEBHOOK_CHECK_INTERVAL = 60.0  # seconds between regular checks
WEBHOOK_REPAIR_MAX_ATTEMPTS = 5
WEBHOOK_REPAIR_BASE_DELAY = 1.0  # base for exponential backoff
...
async def ensure_webhook(_wh_url):
    async with _webhook_monitor_lock:
        ...
        # Webhook ok
        wh_logger.debug(f'Webhook OK: {current_url}')
        return True
...
async def _webhook_monitor_loop(_wh_url):
    ...
    while not _webhook_monitor_stop.is_set():
        try:
            ...
            wh_logger.info('Webhook monitor cancelled')
            break
        except Exception as e:
...
async def stop_webhook_monitor():
    ...
    if _webhook_monitor_task is not None:
        ...
        try:
            ...
            pass
    wh_logger.info('Webhook monitor stopped')
